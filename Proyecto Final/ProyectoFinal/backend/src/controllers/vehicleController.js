...
export async function deleteVehicle(req, res) {
  try {
    ...
    if (result.error) {
      const statusCode =
        result.error.code === 'VEHICLE_HAS_PRODUCTS' ? 400 : 404;
...
export async function addProductToVehicle(req, res) {
  try {
    ...
    const result = await VehicleService.addProductToVehicle(
      vehicleId,
      productId
    );
    ...
    if (result.error) {
      const statusCode =
        result.error.code === 'VEHICLE_NOT_FOUND' ||
...
export async function removeProductFromVehicle(req, res) {
  try {
    const { vehicleId, productId } = req.body;
    const result = await VehicleService.removeProductFromVehicle(
      vehicleId,
      productId
    );
